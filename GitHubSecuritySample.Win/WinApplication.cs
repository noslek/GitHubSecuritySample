...
public class GitHubSecuritySampleWindowsFormsApplication : WinApplication {
    public GitHubSecuritySampleWindowsFormsApplication() {
        SplashScreen = new DXSplashScreen(typeof(XafSplashScreen), new DefaultOverlayFormOptions());
        ApplicationName = "GitHubSecuritySample";
        CheckCompatibilityType = DevExpress.ExpressApp.CheckCompatibilityType.DatabaseSchema;
        UseOldTemplates = false;
        DatabaseVersionMismatch += GitHubSecuritySampleWindowsFormsApplication_DatabaseVersionMismatch;
        CustomizeLanguagesList += GitHubSecuritySampleWindowsFormsApplication_CustomizeLanguagesList;
    }
    private void GitHubSecuritySampleWindowsFormsApplication_CustomizeLanguagesList(object sender, CustomizeLanguagesListEventArgs e) {
        string userLanguageName = System.Threading.Thread.CurrentThread.CurrentUICulture.Name;
        if(userLanguageName != "en-US" && e.Languages.IndexOf(userLanguageName) == -1) {
            e.Languages.Add(userLanguageName);
        }
    }
    private void GitHubSecuritySampleWindowsFormsApplication_DatabaseVersionMismatch(object sender, DevExpress.ExpressApp.DatabaseVersionMismatchEventArgs e) {
